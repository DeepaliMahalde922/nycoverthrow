...
function booked_fe_calendar_date_content_trainer($traner_slug = false, $calendar_id = false){
	$final_html= 'true';
	...
	foreach ($terms as $key => $value) {
		...
		$calendar_id = $value->term_id;
		
		if( $calendar_id == 371 || $calendar_id == 370 ){
			...
			for ($i=0; $i < 7; $i++) {
				...
				if($i == 0){
					...
					$cur_epch = strtotime($prev_date);
				}

				$cur_epch = $cur_epch + 86400;
				$date = date("Y-m-d", $cur_epch);
				...
				if ($todays_defaults){
					...
					foreach($todays_defaults as $timeslot => $count):
						...
						/* Are there any appointments in this particular timeslot? If so, let's create an array of them. */
										...
										if( isset($available_trainer) && is_array($available_trainer) && count($available_trainer) > 0 ){
											...
											if( $twenty_four_format != 'not applicable' && array_key_exists($twenty_four_format, $available_trainer) ){
												...
												if( isset($trainer_info_exploded) && is_array($trainer_info_exploded) && count($trainer_info_exploded) >= 3 ){
													...
													$alltraner 	 = sanitize_title($traner_name);
													if( $traner_slug == $alltraner ){
														...
														if(function_exists('ovr_get_portfolio_page_by_trainer_name')){	
															...
															$term = get_term( $calendar_id, 'booked_custom_calendars' );
															$termname = $term->name;
															$timestamp = strtotime($date);
															$day1 =  date("l", $timestamp);
															$html = '<div class="item booked-appt-list"><div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
															$html.= '<a class="trainer_bookie" href="javascript:void(0)">';
															$trainer_html.= '<label>'.$traner_name.'</label>';
															$trainer_html.= '<h3>'.$termname.'</h3>';
															$trainer_html .= '<span class="timeslot-range"><i class="booked-icon booked-icon-clock"></i>&nbsp;&nbsp;' . $timeslotText . '</span>';
															$trainer_html .= '<span class="timeslot-day">' . $day1 . '</span>';
															$trainer_html .= '<span class="timeslot-people"><button data-calendar-id="'.$calendar_id.'" data-title="'.esc_attr($title).'" data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available || !$available ? ' disabled' : '').'>'.( $title ? '<span class="timeslot-mobile-title">'.esc_html($title).'</span>' : '' ).'<span class="button-timeslot">'.apply_filters('booked_fe_mobile_timeslot_button',$timeslotText,$this_timeslot_timestamp,$timeslot,$calendar_id).'</span>'.apply_filters('booked_button_book_appointment', '<span class="button-text">Select</span>' . ( !$hide_available_count ? '' : '' ) ).'</button></span>';
												    		$html.= $trainer_html;
															$html.= '</a></div></div>';
															$final_html = 'false';
					...
					if (!$temp_count):
						echo '<p>'.esc_html__('There are no class time slots available for this day.','booked').'</p>';
					endif;
	...
	if($final_html == 'true'){
		//$appt_list_html = '<h3>There are no class time slots available for this trainer.</h3>';
		$appt_list_html = 'false';
		echo $appt_list_html;	
	}else{
		echo $appt_list_html;
	}
	/*if($appt_list_html){
		...
		if( $final_html == 'true' ){
			$appt_list_html = '<div class="no-classes">'.esc_html__('There are no class time slots available for this trainer.','booked').'</div>';
		}
	}*/
}
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	$cururl     = wp_get_referer();
	$namelist =  explode('/', $cururl);

	if( strpos( $cururl, 'trainer' ) !== false ) {
		booked_fe_calendar_date_content_trainer($date, $calendar_id);
		die();
	}
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$html = '<div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							...
							/*== Added by Rahul Start =*/
							if( $calendar_id ){
								$html .= ovr_get_google_calender($year, $month, $day, $calendar_id, $timeslotText);
							}
							/*== Added by Rahul End =*/
		...
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
		    ...
		    if ( isset( $_POST['nickname'] ) ):
   		    ...
   		    if ( isset( $_POST['first_name'] ) ):
   		        update_user_meta( $booked_current_user->ID, 'first_name', esc_attr( $_POST['first_name'] ) );
   		    endif;

   		    if ( isset( $_POST['last_name'] ) ):
   		        update_user_meta( $booked_current_user->ID, 'last_name', esc_attr( $_POST['last_name'] ) );
   		    endif;

   		    if ( isset( $_POST['first_name'] ) || isset( $_POST['last_name'] ) ):
   		    	$full_name = $_POST['first_name'].' '.$_POST['last_name'];

   		    	update_user_meta( $booked_current_user->ID, 'nickname', esc_attr( $full_name ) );
   		    	wp_update_user( array ('ID' => $booked_current_user->ID, 'display_name' => esc_attr( $full_name )));
   		    endif;
...
function booked_profile_content_edit(){
    ...
    <form method="post" enctype="multipart/form-data" id="booked-page-form" action="<?php the_permalink(); ?>">
        ...
        <div class="bookedClearFix">
            ...
            <p class="form-password form-first_name">
                <label for="first_name"><?php esc_html_e('First Name', 'booked'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (BOOKED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="first_name" type="text" id="nickname" value="<?php the_author_meta( 'first_name', $booked_current_user->ID ); ?>" />
            </p><!-- .form-first_name -->
            <p class="form-password form-last_name last">
               <label for="last_name"><?php esc_html_e('Last Name', 'booked'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (BOOKED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="last_name" type="text" id="nickname" value="<?php the_author_meta( 'last_name', $booked_current_user->ID ); ?>" />
            </p><!-- .form-last_name -->
