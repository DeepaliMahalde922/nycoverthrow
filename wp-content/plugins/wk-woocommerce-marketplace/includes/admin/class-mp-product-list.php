...
class MP_Product_List{
	...
	public function __construct( $product ) {
		...
		elseif ( $product instanceof MP_Product_List ) {

			$this->id   = absint( $product->id );
			$this->post = $product;
		}
		elseif ( $product instanceof WP_Post || isset( $product[0]->ID ) ) {
			$this->id   = absint( $product[0]->ID );
			$this->post = $product;
		}
	}
			...
			if ( $this->is_in_stock() ) {
				...
				if ( $this->get_total_stock() > get_option( 'woocommerce_notify_no_stock_amount' ) ) {
					...
					switch ( $format_option ) {
						case 'no_amount' :
							$format = __( 'In stock', 'marketplace' );
						break;
						case 'low_amount' :
							$low_amount = get_option( 'woocommerce_notify_low_stock_amount' );

							$format = ( $this->get_total_stock() <= $low_amount ) ? __( 'Only %s left in stock', 'marketplace' ) : __( 'In stock', 'marketplace' );
						break;
						default :
							$format = __( '%s in stock', 'marketplace' );
						break;
					}
					...
					if ( $this->backorders_allowed() && $this->backorders_require_notification() ) {
						$availability .= ' ' . __( '(backorders allowed)', 'marketplace' );
					}
					...
					if ( $this->backorders_allowed() ) {
						if ( $this->backorders_require_notification() ) {
							$availability = __( 'Available on backorder', 'marketplace' );
							$class        = 'available-on-backorder';
						} else {
							$availability = __( 'In stock', 'marketplace' );
						}
					} else {
						$availability = __( 'Out of stock', 'marketplace' );
						$class        = 'out-of-stock';
					}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->get_price() > 0 ) {
			...
			if ( $this->is_on_sale() && $this->get_regular_price() ) {
				...
				$price .= $this->get_price_html_from_to( $display_regular_price, __( 'Free!', 'marketplace' ) );

				$price = apply_filters( 'woocommerce_free_sale_price_html', $price, $this );

			} else {

				$price = __( 'Free!', 'marketplace' );
	...
	public function get_price_html_from_text() {
		return '<span class="from">' . _x( 'From:', 'min_price', 'marketplace' ) . ' </span>';
	}
	...
	public function get_rating_html( $rating = null ) {
		...
		if ( $rating > 0 ) {
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated %s out of 5', 'marketplace' ), $rating ) . '">';

			$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'marketplace' ) . '</span>';
	...
	public function get_formatted_name() {
		...
		return sprintf( __( '%s &ndash; %s', 'marketplace' ), $identifier, $this->get_title() );
	}
	public function get_seller_product()
	{
	global $wpdb;
	$product_id=$wpdb->get_results("select ID from $wpdb->posts where post_type='product'");
	return $product_id;
	}
}//$product->get_seller_product();
