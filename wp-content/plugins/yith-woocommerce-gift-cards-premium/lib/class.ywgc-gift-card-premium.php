	...
	class YWGC_Gift_Card_Premium extends YWGC_Gift_Card {
		...
		public function __construct( $args = array() ) {
			...
			if ( is_numeric( $args ) || is_string( $args ) ) {
				$post = is_numeric( $args ) ? get_post( $args ) : get_page_by_title( $args, OBJECT, YWGC_CUSTOM_POST_TYPE_NAME );

				if ( $post ) {
					$args = get_post_meta( $post->ID, YWGC_META_GIFT_CARD_USER_DATA, true );

					$args['ID']               = $post->ID;
					$args['gift_card_number'] = $post->post_title;
					$args['product_id']       = $post->post_parent;
