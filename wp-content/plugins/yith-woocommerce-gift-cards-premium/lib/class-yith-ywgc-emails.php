	...
	class YITH_YWGC_Emails {
		...
		public function show_gift_card_expiration( $gift_card ) {
			if ( $gift_card->expiration ) {
				$message = apply_filters ( 'yith_ywgc_gift_card_email_expiration_message',
					sprintf ( _x ( 'This gift card code will be valid until %s (Y-m-d)', 'gift card expiration date', 'yith-woocommerce-gift-cards' ), date ( 'Y-m-d', $gift_card->expiration ) ) );
		...
		public function show_link_for_cart_discount( $gift_card ) {
			...
			wc_get_template ( 'emails/automatic-discount.php',
				array(
					'apply_discount_url' => apply_filters ( 'yith_ywgc_email_automatic_cart_discount_url', $apply_discount_url ),
				),
				'',
