		...
		public function build_cart_item_data() {
			...
			if ( $product_as_present ) {
				...
				if ( $ywgc_is_manual_amount ) {
					$ywgc_amount = number_format( (float) $_REQUEST['ywgc-manual-amount'], wc_get_price_decimals(), '.', '' );
...
//			$recipient_count = 1;
			...
			if ( $item_data['ywgc_is_digital'] ) {
				...
				if ( $recipient_count > 1 ) {
					foreach ( $item_data['ywgc_recipients'] as $recipient ) {
						$item_data['ywgc_recipients'] = array( $recipient );
						
						$added_to_cart = WC()->cart->add_to_cart( $product_id, 1, 0, array(), $item_data );
					}
				} else {
