...
class WC_Product_Booking extends WC_Product {
	...
	public function get_resource( $id ) {
		...
		if ( $id ) {
			$transient_name = 'book_res_' . md5( http_build_query( array( $id, WC_Cache_Helper::get_transient_version( 'bookings' ) ) ) );
	 ...
	 * @return array
	 */
	public function get_availability_rules( $for_resource = 0, $date = null ) {
		if ( empty( $this->availability_rules[ $for_resource ] ) ) {
			...
			$availability_rules =
				array_filter( 
					array_reverse( 
						array_merge( 
							WC_Product_Booking_Rule_Manager::process_availability_rules( $resource_rules, 'resource', $date ),
							WC_Product_Booking_Rule_Manager::process_availability_rules( $product_rules, 'product', $date ),
							WC_Product_Booking_Rule_Manager::process_availability_rules( $global_rules, 'global', $date ) 
						)
					)
				);
			...
			switch ( $type ) {
				...
				case 'time:7':
					if ( false === $default_availability && ( $day_of_week === $rules['day'] || 0 === $rules['day'] ) ) {
						$bookable = $rules['rule'];
						// removed break 2 refer to
						// https://github.com/woothemes/woocommerce-bookings/issues/646
					}
					break;
				case 'time:range':
