...
function bawllm_setup_nav_menu_item( $item ) {	
	...
	if ( $pagenow != 'nav-menus.php' && ! defined( 'DOING_AJAX' ) && isset( $item->url ) && strstr( $item->url, '#baw' ) != '' ) {
		...
		if (strpos($_SERVER['REQUEST_URI'],'my-account') !== false) {		    
			...
			$referer_url = "?referer=".site_url().$_SERVER['REQUEST_URI'];
		}
		...
		switch ( $item_url ) {
			case '#bawloginout#' : 	
									...
									//$item->url = is_user_logged_in() ? wp_logout_url( site_url() ) : wp_login_url( $item_redirect[0] );
									$item->url = is_user_logged_in() ? wp_logout_url( site_url().$_SERVER['REQUEST_URI'] ) : site_url()."/my-account".$referer_url;
									$item->title = bawllm_loginout_title( $item->title ) ; break;
			//case '#bawlogin#' : 	$item->url = wp_login_url( $item_redirect ); break;
			case '#bawlogin#' : 	$item->url = site_url()."/my-account".$referer_url; break;
			case '#bawlogout#' : 	$item->url = wp_logout_url( site_url() ); break;
