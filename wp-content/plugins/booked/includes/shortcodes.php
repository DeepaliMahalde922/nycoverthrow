					...
					foreach($appointments_array as $appt):
						...
						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.$appt['post_id'].'">';
							...
							if (!$historic):
								...
								* Modified by TInfini
								**/
								$appt_changed_count = get_post_meta( $appt['post_id'], 'ovr_reschedule_count', true );
								if( empty( $appt_changed_count ) ){
									update_post_meta( $appt['post_id'], 'ovr_reschedule_count', 0 );
									$appt_changed_count = 0;
								}
								if( $appt_changed_count < 1 ){
									if ($buttons_content):
										echo '<div class="booked-cal-buttons">';
											echo $buttons_content;
											echo '<p class="ovr_reschedule_note">*Classes may only be changed once.</p>';
										echo '</div>';
									endif;
								} else {
									echo '<p class="ovr_class_date_changed">Original class date previously changed.</p>';
								}
								// End! - TInfini
