	...
	class YWCCP_Admin {
		...
		public function plugin_fw_loader() {
			...
			if ( ! defined( 'YIT_CORE_PLUGIN' ) ) {
				require_once( YWCCP_DIR . '/plugin-fw/yit-plugin.php' );
			}
		}
		...
		public function enqueue_scripts() {
			...
			//if ( isset( $_GET['page'] ) && $_GET['page'] == 'ywccp_panel' ) {
		...
		public function save_options() {
			...
			for( $i = 0; $i <= $max; $i++ ){
				...
				$new_fields[ $name ]['options']     = ! empty( $_POST['field_options'][ $i ] ) ? $this->crete_options_array( $_POST['field_options'][ $i ] ) : array();
				...
				$new_fields[ $name ]['required']    = ! empty( $_POST['field_required'][ $i ] ) ? true : false;
		 ...
		 * @return array
		 */
		protected function crete_options_array( $options ) {
			...
			// first of all add empty options for placeholder
			$options_array[''] = '';
...
/**
 * Unique access to instance of YITH_Customize_Checkout_Admin class
 *
 * @return \YWCCP_Admin
 * @since 1.0.0
 */
