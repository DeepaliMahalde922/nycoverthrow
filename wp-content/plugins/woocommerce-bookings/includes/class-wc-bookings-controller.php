...
class WC_Bookings_Controller {
	 ...
	 */
	public static function find_booked_day_blocks( $product_id, $date = null ) {
		...
		$existing_bookings = WC_Bookings_Controller::get_bookings_for_objects( $find_bookings_for, $date );
		...
		// Use the existing bookings to find days which are partially or fully booked
	 ...
	 */
	public static function get_bookings_for_objects( $ids = array(), $date = null ) {
		$transient_name = 'book_fo_' . md5( http_build_query( array( $ids, $date, WC_Cache_Helper::get_transient_version( 'bookings' ) ) ) );
		$bookings = array();

		if ( false === ( $booking_ids = get_transient( $transient_name ) ) ) {
			$booking_ids = self::get_bookings_for_objects_query( $ids, $date );
			set_transient( $transient_name, $booking_ids, DAY_IN_SECONDS * 30 );
		}
	 ...
	 * @return array of WC_Booking objects
	 */
	public static function get_bookings_for_objects_query( $ids, $date = null ) {
		global $wpdb;

		$status = array( 'confirmed', 'paid' );

		// if no date set, we assume it is today's date till last day of the month
		if ( null === $date ) {
			$start_date = date( 'Ymd000000' );
			$end_date   = date( 'Ymd240000', strtotime( 'last day of this month' ) );
		} else {
			$start_date = date( 'Ymd000000', strtotime( 'first day of ' . $date ) );
			$end_date   = date( 'Ymd2400000', strtotime( 'last day of ' . $date ) );
		}
		...
		$booking_ids = $wpdb->get_col( "
			...
			LEFT JOIN {$wpdb->postmeta} as _start_date ON {$wpdb->posts}.ID = _start_date.post_id
			LEFT JOIN {$wpdb->postmeta} as _end_date ON {$wpdb->posts}.ID = _end_date.post_id
			WHERE post_type = 'wc_booking'
			...
			AND _start_date.meta_key = '_booking_start'
			AND _end_date.meta_key = '_booking_end'
			AND (
			...
			AND _start_date.meta_value >= '{$start_date}'
			AND _end_date.meta_value <= '{$end_date}'
		" );
