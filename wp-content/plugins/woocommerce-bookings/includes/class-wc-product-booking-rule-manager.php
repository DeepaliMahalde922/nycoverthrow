...
class WC_Product_Booking_Rule_Manager {
	 ...
	 * @return array
	 */
	public static function process_availability_rules( $rules, $which, $date = null ) {
		...
		foreach ( $rules as $fields ) {
			...
			// skip dates that are not in current calendar date view
			if ( null === $date && ! empty( $fields['from_date'] ) && ! empty( $fields['to_date'] ) ) {
				// skip dates that are in the past
				if ( strtotime( $fields['from_date'] ) <= strtotime( 'first day of this month' ) && strtotime( $fields['to_date'] ) <= strtotime( 'first day of this month' ) ) {
					continue;
				}

				// skp dates that are in the future
				if ( strtotime( $fields['from_date'] ) >= strtotime( 'last day of this month' ) && strtotime( $fields['to_date'] ) >= strtotime( 'last day of this month' ) ) {
					continue;
				}
			} elseif ( ! empty( $fields['from_date'] ) && ! empty( $fields['to_date'] ) ) {
				// skip dates that are in the past
				if ( strtotime( $fields['from_date'] ) <= strtotime( 'first day of ' . $date ) && strtotime( $fields['to_date'] ) <= strtotime( 'first day of ' . $date ) ) {
					continue;
				}

				// skip dates that are in the future
				if ( strtotime( $fields['from_date'] ) >= strtotime( 'last day of ' . $date ) && strtotime( $fields['to_date'] ) >= strtotime( 'last day of ' . $date ) ) {
					continue;
				}
			}
