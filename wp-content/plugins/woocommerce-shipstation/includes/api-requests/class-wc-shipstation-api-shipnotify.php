...
class WC_Shipstation_API_Shipnotify extends WC_Shipstation_API_Request {
	...
	public function request() {
		...
		if ( sizeof( $shipped_items ) > 0 ) {
			...
			$current_shipped_items = max( get_post_meta( $order_id, '_shipstation_shipped_item_count', true ), 0 );

			if ( ( $current_shipped_items + $shipped_item_count ) >= $total_item_count ) {
				$order_shipped = true;
			}

			$this->log( sprintf( __( "Shipped %d out of %d items in order %s", 'woocommerce-shipstation' ), $shipped_item_count, $total_item_count, $order_id ) );

			update_post_meta( $order_id, '_shipstation_shipped_item_count', $current_shipped_items + $shipped_item_count );

		// If we don't have items from SS and order items in WC, just complete the order as a whole
		} else {
			$order_shipped = 0 === $total_item_count;

			$order_note    = sprintf( __( 'Items shipped via %s on %s with tracking number %s (Shipstation).', 'woocommerce' ), esc_html( $carrier ), date_i18n( get_option( 'date_format' ), $timestamp ), $tracking_number );
		...
		if ( $order_shipped ) {
			...
			$this->log( sprintf( __( "Updated order %s to status %s", 'woocommerce-shipstation' ), $order_id, WC_ShipStation_Integration::$shipped_status ) );
		}
