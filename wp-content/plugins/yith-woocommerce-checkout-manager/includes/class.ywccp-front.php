...
if ( ! class_exists( 'YWCCP_Front' ) ) {
	...
	class YWCCP_Front {
		...
		public function datepicker_type( $field, $key, $args, $value ){
			...
			<input name="<?php echo esc_attr( $key ) ?>" id="<?php echo esc_attr( $args['id'] ) ?>" type="text" class="ywccp-datepicker-type" value="<?php echo $value ?>" placeholder="<?php echo esc_attr( $args['placeholder'] )?>">
			...
			foreach ( $checkout_fields as $fieldset_key => $fieldset ) {
				foreach ( $fieldset as $key => $field ) {
					if ( isset( $posted[ $key ] ) ) {
						...
						if( $field['type'] == 'checkbox' && isset( $field['required'] ) && $field['required'] && ! $posted[$key] ) {
							...
							foreach ( $field['validate'] as $rule ) {
								switch ( $rule ) {
									case 'vat' :
										...
										// validate vat
										$this->validate_vat_field( $posted[ $key ], $posted['billing_country'] );
										break;
									default :
										if( $rule )
											do_action( 'ywccp_validation_field_' . $rule, $posted );
										break;
								}
							}
						}
...
/**
 * Unique access to instance of YITH_Customize_Checkout_Front class
 *
 * @return \YWCCP_Front
 * @since 1.0.0
 */
